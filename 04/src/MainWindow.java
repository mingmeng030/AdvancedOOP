...
public class MainWindow extends FrameWindow {
    ...
    public JFrame createWindow(String title, int x, int y, int width, int height) {
        JFrame frame;// 전체 화면의 뼈대를 구성함 (윈도우 틀을 만들어줌)
        frame = new JFrame(title);// 화면 크기 지정 (윈도우 화면의 너비와 높이는 매개 변수로 전달됨)
        frame.setBounds(x, y, width, height);
        
        // createPanel() 함수를 호출해서 파일에서 읽은 디스플레이 구성 요소에 따라 디스플레이를 조합, 전체 디스플레이를 구성하는 패널을 돌려 받음
        // 실질적인 화면 구성은 createPanel()함수에서 일어남
        JPanel panel = createPanel(width, height);

        frame.getContentPane().add(panel);  // 윈도우에 전체 패널을 붙임
        frame.pack();
    ...
    public JPanel createPanel(int width, int height) {
        JPanel panel = new JPanel(); // 제일 바탕에 놓일 패널 생성
        ...
        Display display = new HudDisplay(WIDTH, HEIGHT);// 기본 컴포넌트 생성. HudDisplay는 가장 기본적인 디스플레이 구성요소가 됨
        ...
        for (String name : displayList) {
            if (name.equals("speed")) // display 변수가 참조하는 기존 디플레이에 속도계 디스플레이를 덧 붙임
                display = new SpeedometerDisplay(display, WIDTH, HEIGHT);

            else if (name.equals("time")) // display 변수가 참조하는 기존 디플레이에 시계 디스플레이를 덧 붙임
                display = new TimeDisplay(display, WIDTH, HEIGHT);

            else if (name.equals("weather")) // display 변수가 참조하는 기존 디플레이에 날씨 디스플레이를 덧 붙임
        ...
        System.out.println("disply.height = " + display.getHeight());   // 높이를 출력
        display.show();        // 디스플레이마다 각각의 내용을 화면에 보임
        return panel;
    }
